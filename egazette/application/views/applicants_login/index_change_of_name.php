    ...
    <div class="page page-tables-footable">
        ...
        <div class="col-md-12">
                <section class="boxs box_report <?php echo $class ?>" id="show_filter_form">
                    ...
                    <?php echo form_open('applicants_login/search_for_change_of_name_surname', array('method' => 'post','id'=>'search_data', 'autocomplete' => 'off')); ?>
                        <div class="boxs-body">
                            <div class="row">
                                    ...
                                    <select name="status" id="status" class="form-control">
                                                ...
                                                <?php if (!empty($total_status)) { ?>
                                                    <?php foreach ($total_status as $sname) { ?>
                                                        <option value="<?php echo $sname->id; ?>" <?php if($status == $sname->id){ echo 'selected'; } ?> ><?php echo $sname->status_name; ?></option>
                                                    <?php } ?>
                                                <?php } ?>
                                        </select>
                                </div>
                        ...
                        <div class="row">
                            <div class="col-sm-5">
                                <?php if ($this->session->userdata('is_admin')) { ?>
                                    <h3 class="custom-font hb-blue"><strong>Govt. Press Change of Name/Surname Details</strong></h3>
                                <?php } else { ?>
                                    <h3 class="custom-font hb-blue"><strong>Change of Name/Surname Details</strong></h3>
                                <?php } ?>
                            </div>
                            <div class="col-sm-5">
                                <?php echo form_open('applicants_login/search_for_change_of_name_surname', array('method' => 'post','id'=>'search_data', 'autocomplete' => 'off')); ?>
                                    <div class="form-group col-sm-4 form-group-select">
                                        <label for="status">Status </label>
                                        <select name="status" id="status" class="form-control">
                                            <?php 
                                            if (!empty($total_status)) {
                                                $allowed_ids = array(1, 2, 18); // Specify the IDs you want to display
                                                foreach ($total_status as $sname) {
                                                    if (in_array($sname->id, $allowed_ids)) { // Check if the status ID is in the allowed list
                                            ?>
                                                        <option value="<?php echo $sname->id; ?>" <?php if($status == $sname->id){ echo 'selected'; } ?>><?php echo $sname->status_name; ?></option>
                                            <?php
                                                    }
                            ...
                            <div class="col-sm-2 text-right">
                                <button class="btn filter-margin btn-success btn-raised btn-fab btn-fab-mini btn-round" id="show_filter" title="Filter">
                                    <i class="fa fa-filter"></i>
                                </button>
                                <?php if ($this->session->userdata('is_applicant')) { ?>
                                    <a href="<?php echo base_url(); ?>applicants_login/add_change_of_name_surname" class="btn-bg btn btn-success btn-right-align">Add</a>
                                <?php } ?>
                            </div>
                        </div>
                    ...
                    <div class="boxs-body">
                                ...
                                <?php if (!empty($change_of_names)) { ?>
                                    ...
                                    ?>
                                        ...
                                        <tr>
                                            ...
                                            <td class="center">
                                                <a href="<?php echo base_url(); ?>applicants_login/view_details_name_surname/<?php echo $data->id; ?>"><i class="fa fa-eye"></i></a>
                                            </td>
                                        </tr>
                                        <?php $cntr++;
                                    } ?>
                        ...
                        <?php if (isset($links)) { ?>
                            ...
                            <div class="go-to-page">
                                ...
                                <input type="number" id="pageNumber" min="1" max="<?php echo ceil($config["total_rows"] / $config["per_page"]); ?>" />
...
<script type="text/javascript" nonce="a6b9a780936c8e980939086f618dded2">
    $(document).ready(function(){
        ...
        $('#reset_btn').click(function () { 
            ...
            window.location.href = "<?php echo base_url(); ?>applicants_login/index_for_change_of_name_surname"           
        });
        ...
        $('#search_name').click(function(){
            ...
            if (data === "" && name_data === "" && s_data === "" && f_data === "" && t_data === "") { 
...
<script>
    ...
    function resetInactivityTimeout() {
        clearTimeout(inactivityTimeout);
        inactivityTimeout = setTimeout(function() {
            window.location.href = "<?php echo base_url('applicants_login/logout_router'); ?>";
        }, 15 * 60 * 1000);
    }
...
<script>
    document.getElementById('goToPageButton').addEventListener('click', function () {
        ...
        if (pageNumber) {
            const url = "<?php echo base_url('applicants_login/index_for_change_of_name_surname/'); ?>" + pageNumber;
