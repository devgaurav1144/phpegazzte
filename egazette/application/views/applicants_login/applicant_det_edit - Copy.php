...
<!-- CONTENT -->
<script src="<?php echo base_url(); ?>assets/js/vendor/html5lightbox/html5lightbox.js" nonce="a6b9a780936c8e980939086f618dded2"></script>
    ...
    <?php } ?>  
           ...
           <?php echo form_open("https://uat.(StateName)treasury.gov.in/echallan/dept-intg", array("method" => "POST", "id" => "form2", "name" => "form2")); ?>
				<?php 
					...
					// Depositor Address
					$depositor_addr = $gz_dets->address;
					// State
					$state = "(StateName)";
					// District
					$district = "Khudha";
        ...
        <?php if($this->session->userdata('is_applicant') && $gz_dets->cur_status == 13) { ?>
            <div class="row">
                <div class="col-md-12">
                    <section class="boxs">
                        <div class="boxs-header">
                        ...
                        <?php echo form_open('', array('name' => 'form_pa',  'id' => 'form_pa', 'method' => 'post', 'enctype' => 'multipart/form-data')); ?>
                            <div class="boxs-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="form-group col-md-4">
                                        <label for="address_3">Address 3 : <span class="asterisk">*</span></label>
                                        <input type="text" id="address_3" name="address_3" placeholder="Enter Address 3" class="form-control" maxlength="50" autocomplete="off" value="<?php echo $gz_dets->address_3 ?>"/>
                                        <?php if (form_error('address_3')) { ?>
                                            <span class="error"><?php echo form_error('address_3'); ?></span>
                                        <?php } ?>
                                    </div>
                                </div>
                                <?php $file_type = ""; $i = 1; $j = 1; ?>

                                <?php foreach ($tot_docus as $tot_docu) { ?>
                                    <?php $file_type  = "image/*"?>

                                    <?php if($tot_docu->document_name =='PAN Card of Partners') { ?>
                                        
                                            <div class="form-group col-md-12" id="app_pan">
                                                <div  class="col-md-6">

                                    <?php } else if($tot_docu->document_name =='Aadhaar Card of Partners')  { ?>
                                        
                                            <div class="form-group col-md-12" id="app_aadhar">
                                                <div  class=" col-md-6">
                                    ...
                                    <?php  } ?>
                                            ...
                                            <div class="fileupload-buttonbar clearfix">
                                                ...
                                                <?php if($tot_docu->document_name =='PAN Card of Partners') { ?>
                                                    ...
                                                    <a id="<?php echo $tot_docu->id; ?>" class="btn_add_pan ">
                                                    ...
                                                    <div class="fileupload-buttonbar clearfix">   
                                                        ...
                                                        <?php if($i == 1 ) { ?>
                                                            <?php $image_pan = $this->Applicants_login_model->select_cur_path_pan($par_id); ?>
                                                            <?php foreach($image_pan as $image ) {  ?>

                                                                <input type="hidden" id="img_id_<?php echo $tot_docu->id.'_'.$i; ?>" name ="img_id_<?php echo $tot_docu->id; ?>[]" value="<?php if(!empty($image->pan_card_of_partnetship)) {  echo  $image->pan_card_of_partnetship;  } ?>"/>
                                                ...
                                                <?php } else if($tot_docu->document_name =='Aadhaar Card of Partners')  { ?>
                                                    ...
                                                    <a id="<?php echo $tot_docu->id; ?>" class="btn_add_aadhar ">
                                                        <i class="fa fa-plus" ></i>
                                                    </a>
                                                    ...
                                                    <?php if($j == 1 ) { ?> 
                                                        ...
                                                        <div class="fileupload-buttonbar clearfix" >
                                                            <?php $images_aadhar = $this->Applicants_login_model->select_cur_path_aadhar($par_id); ?>
                                                                <?php foreach($images_aadhar as $image ) {  ?>

                                                                    <input type="hidden" id="img_id_<?php echo $tot_docu->id.'_'.$j;?>" name ="img_id_<?php echo $tot_docu->id; ?>[]" value="<?php if(!empty($image->aadhar_card_of_partnetship)) {  echo  $image->aadhar_card_of_partnetship;  } ?>"/>
                                                                <?php  $j++; } ?>
                                                        </div>
                                                    <?php } ?>
                                                ...
                                                <?php  } else { ?>
                                                    ...
                                                    <?php $images = $this->Applicants_login_model->select_cur_path($par_id); ?>
                                                    <?php foreach($images as $image ) { ?>
                                                        <?php if($tot_docu->document_name == 'Original Partnership Deed') { ?>

                                                            <input type="hidden" id="img_id_<?php echo $tot_docu->id; ?>" name ="img_id_<?php echo $tot_docu->id; ?>[]"  value ="<?php if(!empty($image->orignal_partnership_deed)) {  echo  $image->orignal_partnership_deed;  } ?>"/>

                                                        <?php } else if($tot_docu->document_name == 'Deed of Reconstitution of Partnership') { ?>

                                                            <input type="hidden" id="img_id_<?php echo $tot_docu->id; ?>" name ="img_id_<?php echo $tot_docu->id; ?>[]" value ="<?php if(!empty($image->deed_of_reconstitution_of_partnership)) { echo  $image->deed_of_reconstitution_of_partnership; } ?>"/>

                                                        <?php } else if(trim($tot_docu->document_name) == 'IGR Certificate') { ?>

                                                            <input type="hidden" id="img_id_<?php echo $tot_docu->id; ?>" name ="img_id_<?php echo $tot_docu->id; ?>[]"  value ="<?php if(!empty($image->igr_certificate_file)) { echo  $image->igr_certificate_file; } ?>"/>

                                                        <?php } else if($tot_docu->document_name == 'Original Newspaper Advertisement') { ?>

                                                            <input type="hidden" id="img_id_<?php echo $tot_docu->id; ?>" name ="img_id_<?php echo $tot_docu->id; ?>[]" value ="<?php if(!empty($image->orignal_newspaper_of_advertisement)) { echo  $image->orignal_newspaper_of_advertisement; } ?>"/>

                                                        <?php } else if($tot_docu->document_name == 'Notice in Softcopy') { ?>

                                                            <input type="hidden" id="img_id_<?php echo $tot_docu->id; ?>" name ="img_id_<?php echo $tot_docu->id; ?>[]"  value ="<?php if(!empty($image->pdf_for_notice_of_softcopy)) { echo $image->pdf_for_notice_of_softcopy; } ?>"/>
                                    ...
                                    <?php if($tot_docu->document_name =='Aadhaar Card of Partners') { ?>
                                            </div>
                                        </div>
                                    <?php } else if($tot_docu->document_name =='PAN Card of Partners')  { ?>
                                            </div>
                                        </div>
                                    <?php  } ?>
...
$(document).ready(function () {
    $.ajax({
            ...
            data: {
                id: '<?php echo $gz_dets->state_id ?>'
            },
            success: function (data) {

                $('#district_id').html(data);
                <?php if ($gz_dets->district_id != '') { ?>
                    $('#district_id').val('<?php echo $gz_dets->district_id ?>');
                <?php } ?>
                $(".loader").hide();
            }
        });
        ...
        $.ajax({
            ...
            data: {
                id: '<?php echo $gz_dets->district_id ?>'
            },
            success: function (data) {

                $('#police_station_id').html(data);
                <?php if ($gz_dets->police_station_id != '') { ?>
                    $('#police_station_id').val('<?php echo $gz_dets->police_station_id ?>');
                <?php } ?>
                $(".loader").hide();
            }
        });
