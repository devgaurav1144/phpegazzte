            ...
            <div class="col-md-12">
                <section class="boxs">
                    <div class="boxs-header">
                        <h3 class="custom-font hb-blue"><strong>Preview Extraordinary Gazette</strong></h3>
                    ...
                    <div class="gazette-main">
                        <div class="row">
                            <div class="pdf-container">
                                <embed src="<?php echo base_url() . $details->press_pdf_file_path; ?>" type="application/pdf" width="100%" height="650px" internalinstanceid="8">
                            </div>
                        </div>
                    </div>
                    <div class="boxs-footer text-right bg-tr-black lter dvd dvd-top">
                        <?php
                            $pdftext = file_get_contents($details->press_pdf_file_path);
                            $num_pag = preg_match_all("/\/Page\W/", $pdftext, $dummy);
                            $window_path = str_replace('/', '\\', $details->press_pdf_file_path);
                            $pdf_path = substr($details->press_pdf_file_path, 1);
                        ?>
                        <?php if (empty($details->press_signed_pdf_path)) { ?>
							<a href="http://localhost:81/StartPage.aspx?files=<?php echo $pdf_path; ?>&page=<?php echo $num_pag; ?>&gazette_id=<?php echo $details->gazette_id; ?>&type=press&category=extra&signed_name=<?php echo $signed_name; ?>&designation=<?php echo $signed_designation; ?>" class="btn btn-raised btn-info dept_sign_btn" id="sign_pdf_btn">Sign PDF<div class="ripple-container"></div></a>
                        <?php } ?>
                        <!-- Final -->
                        <?php if (!empty($details->press_signed_pdf_path)) { ?>
                            <a href="<?php echo base_url() . $details->press_signed_pdf_path; ?>" class="btn btn-raised btn-success" target="_blank">View Signed PDF<div class="ripple-container"></div></a>
                        <?php } ?>
                        <!-- /Final -->
                        
                        <?php if (!empty($details->press_signed_pdf_path)) { ?>
                            <?php echo form_open('gazette/press_published', array('name' => "sign_form", 'id' => "sign_form", 'role' => "form", 'enctype' => "multipart/form-data", 'method' => "post")); ?>
