<?php
    ...
    class Block extends MY_Controller{
        public function __construct() {
            ...
            $this->load->model(array('block_model'));
         }
        ...
        public function index() {
            ...
            $config["total_rows"] = $this->block_model->get_total_blocks();
            ...
            $data['state_names'] = $this->block_model->get_state_List();
            $state_id = $this->input->post('state_id');
            $data['district_names'] = $this->block_model->get_district_List($state_id);
            $data['blocks'] = $this->block_model->get_block_List($config['per_page'], $offset);
            
            $this->load->view('template/header.php', $data);
            $this->load->view('template/sidebar.php');
            $this->load->view('block/index.php', $data);
        ...
        public function search_block() {
            ...
            $data['title'] = "Search Blocks";
            $config["base_url"] = base_url() . "block/search_block";
            ...
            if($this->input->post()){
                ...
                if($page == 0){
                $array_items = array('state_name', 'district_name', 'block_name');
                ...
                $config["total_rows"] = $this->block_model->get_total_search_blocks($inputs);
                ...
                $dist_id = $this->input->post('district_name');
                
                $data['district_names'] = $this->block_model->get_district_List($state_id);
                $data['blocks'] = $this->block_model->block_search_result($inputs, $config["per_page"], $offset);
            
        // var_dump($data['state_names']);
        // echo $dist_id;
        // echo "<pre>";
        // print_r($data['district_names']);
        // echo "</pre>";
        // exit;
        ...
        public function add() {
            ...
            $this->form_validation->set_rules('block_name', 'Block Name', 'trim|required|max_length[30]');
    
            $data['state_names'] = $this->block_model->get_state_List();
            //$data['district_names'] = $this->block_model->get_district_List();
            ...
            if ($this->form_validation->run() == false) {
                ...
                // insert into DB
                $result = $this->block_model->add($array_data);
    
                // Store Audit Log
               // audit_action_log($this->session->userdata('user_id'), 'Block', 'Add', date('Y-m-d H:i:s', time()), $this->input->ip_address());
                
                $this->session->set_flashdata('success', 'Block added successfully');
                redirect('block/index');
            }
    
            $this->load->view('template/header.php', $data);
            $this->load->view('template/sidebar.php');
            $this->load->view('block/add.php', $data);
            $this->load->view('template/footer.php');
        }

        public function get_district() {
            $state_id = $this->input->post('state_id');
            $distircts = $this->block_model->get_district_List($state_id);
            $output = "";
        ...
        public function edit($id) {
            ...
            if (!is_numeric($id) || !$this->block_model->exists($id)) {
                $this->session->set_flashdata('error', 'Block does not exists');
                redirect('block/index');
            }
    
            $data['bk_dtls'] = $this->block_model->get_block_details($id);
            $state_id = $data['bk_dtls']->state_id;
            ...
            // $this->form_validation->set_rules('state_name', 'State Name', 'trim|required');
            $this->form_validation->set_rules('district_name', 'District Name', 'trim|required');
            $this->form_validation->set_rules('block_name', 'Block Name', 'trim|required|max_length[100]');
            ...
            if ($this->form_validation->run() == false) {
                ...
                // insert into DB
                $result = $this->block_model->edit($array_data);
                if ($result) {
                    ...
                    $this->session->set_flashdata('error', 'Block not updated');
                    redirect('block/index');
                }
            }
            ...
            $this->load->view('template/header.php', $data);
            $this->load->view('template/sidebar.php');
            $this->load->view('block/edit.php', $data);
            $this->load->view('template/footer.php');
        }
        ...
        public function delete() {
            ...
            if ($this->block_model->delete($id)) {
                ...
                $this->session->set_flashdata('error', 'Block not deleted');
                redirect('block/index');
            }
        }
        ...
        public function status_change(){
            ...
            $result = $this->block_model->status_change($id, $status);

            var_dump($result);exit;
            if($result) {
                // Store Audit Log
                audit_action_log($this->session->userdata('user_id'), 'Block', 'Status Change', date('Y-m-d H:i:s', time()), $this->input->ip_address());
